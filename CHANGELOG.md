...
> 状态：已发布（2026-09-10）；[v0.2.43 Release](https://github.com/DeliciousBuding/cloud-path/releases/tag/v0.2.43) 含六平台 18 个二进制与 `checksums.txt`。生产 Server/Edge 与临时 CloudPath 服务继续保持退役，不做部署。
...
> 状态：已发布（2026-09-10）；[v0.2.41 Release](https://github.com/DeliciousBuding/cloud-path/releases/tag/v0.2.41) 含六平台 18 个二进制与 `checksums.txt`。
...
- 应用页面：动作分区排到只读数据之前，主任务不再被「最近一次发生过什么」的卡片压在下面。
- 应用操作回执：布防 / 撤防 / 触发 / 恢复 有了明确的中英文摘要，不再落到「请查看完整结果」的兜底文案。

## v0.2.38 — 2026-09-10

- 设备页数字孪生：设备实时事件映射到模型上的具体元件并短时脉冲高亮，例如 K1/K2/K3、蜂鸣器、数码管、LED、光敏/霍尔/振动传感器；失败和告警使用红色，普通事件使用强调色。
- 操作反馈：命令下发、成功、失败和超时复用同一元件高亮层；平台提示不冒充硬件状态，设备上报仍保持唯一事实源。

## v0.2.37 — 2026-09-10

- 设备页数字孪生：设备实时事件显示为卡片内短时提示与语义色边缘脉冲，失败/告警类事件使用红色，普通事件使用强调色，约 5 秒后自动淡出。
- 操作反馈：命令下发、成功、失败和超时复用同一孪生反馈层；平台提示不冒充硬件状态，设备上报仍保持唯一事实源。

## v0.2.36 — 2026-09-10

- 设备页数字孪生：按设备缓存 WebGL runtime 与场景；在「设备操作」和「概览」之间切换时复用同一画布、相机和材质，不再重建三维场景。
- 设备页加载：3D 空闲预热结果在设备详情模块生命周期内保留，切换分区不再重新等待加载占位；尺寸未变化时跳过重复渲染尺寸配置；图表库仅在打开趋势视图时加载。
- 设备页布局：数字孪生从页头移入「设备操作」和「概览」的三栏右列，默认以右栏小卡展示，展开后占满整行；概览 KPI 优先展示带单位或已声明语义的观测。

> `v0.2.36`–`v0.2.38` 未单独创建 tag/Release；对应变更随 `v0.2.39` 累计发布。

## v0.2.35 — 2026-09-10

- 应用页面状态：设置已停用的应用显示「已停止 · 设置已停用」，不再误报「状态待确认 · 还没有收到运行状态」。
- 应用页面状态：页面顶部只保留一条权威状态（含下一步），删除重复且互相矛盾的状态卡片与停用/过期横幅。
- 应用操作：无需参数的操作按钮直接使用操作名，不再出现「标题 + 通用执行操作按钮」的重复标签。
- 应用概览：指标区块不再渲染没有值的字段，消除无信息量的「未填写」方块。
...
- 副作用幂等：失败的 effect 不再被提前记为已执行，可按同一幂等键重试；并发在途重放保持去重。
- 插件配置：`app_config` / `app_bindings` 在写入口校验 JSON 对象/数组形状，阻止畸形值进入 desired state。
- 插件设置：新建运行项直接按 manifest 渲染结构化字段，普通界面无需手写 `app_config` JSON。

## v0.2.31 — 2026-09-10

- 窄屏布局：为详情页 `Panel` 和标题行补齐 `min-w-0`，修复 390px 下设置数组仍把页面撑到 429px 的残余横向溢出。

## v0.2.30 — 2026-09-10

- 插件设置：结构化配置分组的标题与说明现在按当前语言解析，英文界面不再混入 manifest 中文原文。
- 插件空态：修复 `emptyText` 驼峰字段键在 i18n 索引中被错误小写后无法命中的问题。
- 窄屏布局：修复 390px 下结构化数组配置造成的横向溢出，数组项在设置区内换行收缩。
...
- 访问令牌：重构为通用数据表，补齐筛选、列显隐、批量操作、确认对话框与响应式操作区，保留原有权限和安全边界。
- 设备与趋势：收口详情页、趋势详情和插件应用区块的组件复用与窄屏布局。
- 发布工程：`v0.2.26` 之后的 UIUX 整合全部进入正式 Core Release，供插件页面和应用工作台使用。
...
| `v0.2.43` | 已发布（2026-09-10） | 插件 monorepo catalog、selector、prefix Release 与 tag/lock 绑定；安装/迁移信任硬化；当前平台资产 fail-closed；单插件仓库继续兼容 |
...
| `v0.2.36`–`v0.2.38` | 未单独发布 | 变更随 `v0.2.39` 发布，不提供对应独立 Release |
| `v0.2.35` | 已发布（2026-09-10） | 应用页面信息层级收口：单一权威状态条、停用应用不再误报"状态待确认"、无参操作按钮即操作名、概览不再渲染缺值方块 |
| `v0.2.34` | 已发布（2026-09-10） | 设备页只读 3D 数字孪生接入与时钟走秒 |
| `v0.2.33` | 已发布（2026-09-10） | 应用命令只路由到唯一在线设备，离线候选不再劫持执行器 |
| `v0.2.32` | 已发布（2026-09-10） | 运行时背压/超时与副作用幂等修复，插件配置边界校验和新建结构化表单 |
| `v0.2.31` | 已发布（2026-09-10） | 修复详情页 Panel 作为 grid item 时的残余 390px 横向溢出 |
| `v0.2.30` | 已发布（2026-09-10） | 修复插件设置分组/空态 i18n 与 390px 数组配置横向溢出 |
| `v0.2.29` | 已发布（2026-09-10） | 插件设置结构化配置、schedule records、插件 UI 全量 i18n 与六插件同步发布 |
