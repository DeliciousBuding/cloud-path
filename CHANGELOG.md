...
- 设备页数字孪生：3D 画布跟随应用主题，浅色模式使用亮白背景，深色模式保持棚拍暗色；默认视角改为正面偏上的俯视，模型默认放大展示。
- 设备页布局：数字孪生从页头移入「设备操作」和「概览」的三栏右列，默认以右栏小卡展示，展开后占满整行并让摘要、事件自动换行；概览将摘要、事件、实时孪生与操作记录并入同一弹性骨架，消除空洞。
- 设备概览 KPI：优先展示带单位或已声明语义的观测，Raw ADC 等调试通道降级为兜底，不再挤占温度、光照等业务指标。
