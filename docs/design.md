...
三个只读请求始终使用 desired.instance_id。viewer 可读三个分区，但不能编辑、启停、
重新下发或删除实例。应用数据需要已认证的租户身份，开放访问不等于应用数据授权。

普通视图只展示中文摘要、结构化值、公开 Descriptor / Capability 名称与本地化计划时间。
未知业务枚举保留应用原值，不维护业务专用词典；未知字段以数据项编号呈现，原始字段、
记录分类/标识、任务标识、时间表达式与 JSON 只在技术详情中出现。原有实例配置默认折叠，
不以 app_config 原文作为应用界面。绑定实体缺少公开名称或局部标识存在歧义时，不猜设备归属。
