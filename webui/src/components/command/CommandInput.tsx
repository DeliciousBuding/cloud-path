...
import { commandArgsError, commandFields, unsupportedSchemaKeywords } from '@/lib/command-schema'
...
export function CommandInput({ deviceId, action }: { deviceId: string; action: CommandAction }) {
  ...
  const [json, setJSON] = useState<string | null>(fields ? null : '')
  const built = fieldArgs(fields ?? [], values)
  const args = json ?? (edited ? built.args : '')
  const error = json === null && !edited ? '请填写参数'
    : (json === null ? built.error : undefined) ?? commandArgsError(args, action.inputSchema, action.inputMaxLength)
  ...
  return (
    <fieldset className="min-w-0 border-t border-hairline pt-3" aria-describedby={describedBy}>
      <legend className="mb-1.5 text-[13px] text-ink-2">
        {action.label}{action.hint ? ' · ' + action.hint : ''}
      </legend>
      <p id={descriptionId} className="mb-2 text-[12px] leading-relaxed text-ink-3">
        ≤{argsMaxBytes(action.inputMaxLength)} UTF-8 字节，不含换行/NUL。
      ...
      {fields && json === null ? (
        ...
        <div>
          ...
          <textarea id={id + '-args'} rows={action.inputSchema ? 3 : 2} spellCheck={false}
            ...
            value={args} onChange={(e) => { setJSON(e.target.value); setEdited(true) }}
            placeholder={action.inputSchema ? '填写单行 JSON' : action.inputPlaceholder ?? '参数'}
      ...
      {!edited && error && <p className="mt-2 text-[12px] text-ink-3">填写参数后可下发。</p>}
      {unsupported.length > 0 && <p className="mt-2 break-words text-[12px] text-ink-3">
        本地仅检查 JSON 与已支持的约束；未校验：{unsupported.join('、')}。完整校验以设备端为准。
      </p>}
      ...
      <div className="mt-2 flex flex-wrap items-center justify-end gap-2">
        ...
        <CommandButton deviceId={deviceId} action={action} args={args} disabled={!!error} className="shrink-0" />
      </div>
    </fieldset>
  )
}
