...
export function DeviceRow({ d }: { d: DeviceView }) {
  ...
  return (
    <li className="grid gap-x-4 gap-y-1.5 border-b border-hairline px-4 py-2.5 last:border-b-0 lg:grid-cols-[minmax(0,1.5fr)_minmax(0,0.9fr)_5.5rem_9.5rem] lg:items-center 2xl:grid-cols-[minmax(0,1.5fr)_minmax(0,0.9fr)_minmax(0,1.4fr)_5.5rem_9.5rem]">
      ...
      {/* Edge */}
      <div className="num min-w-0 truncate text-xs text-ink-2" title={`${d.edge_id}${d.adapter ? ` · 设备类型 ${d.adapter}` : ''}${d.port ? ` · ${d.port}` : ''}`}>
      ...
      <div className="flex min-w-0 flex-wrap items-center gap-x-3 gap-y-1 lg:hidden 2xl:flex">
        ...
        {!descriptor ? (
          <span className="text-[12px] text-ink-3" title="设备尚未同步信息，读数未知">等待同步</span>
        ) : metrics.length === 0 ? (
      ...
      {/* Last Seen（绝对时间） */}
      <div className="num min-w-0 truncate text-left font-mono text-[11px] text-ink-3 lg:text-right" title={seen ? fmtDateTime(seen) : '从未更新'}>
        <span className="lg:hidden">{d.online ? '更新于 ' : '最后在线 '}</span>
...
export function DeviceRowHead() {
  return (
    <li
      aria-hidden
      className="hidden gap-x-4 px-4 pb-2 text-[12px] font-medium text-ink-3 lg:grid lg:grid-cols-[minmax(0,1.5fr)_minmax(0,0.9fr)_5.5rem_9.5rem] 2xl:grid-cols-[minmax(0,1.5fr)_minmax(0,0.9fr)_minmax(0,1.4fr)_5.5rem_9.5rem]"
    >
      <span>设备</span><span>网关</span><span className="hidden 2xl:inline">关键数据</span><span>状态</span><span className="text-right">最后在线</span>
    </li>
  )
}
