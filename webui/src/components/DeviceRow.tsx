...
export function DeviceRow({ d }: { d: DeviceView }) {
  ...
  return (
    <li className="grid grid-cols-[minmax(0,1fr)_auto] gap-x-4 gap-y-1 border-b border-hairline px-4 py-3 last:border-b-0 lg:grid-cols-[minmax(0,1.5fr)_minmax(0,0.9fr)_5.5rem_9.5rem] lg:items-center xl:grid-cols-[minmax(0,1.35fr)_minmax(0,0.8fr)_minmax(0,1.2fr)_5.5rem_9.5rem]">
      ...
      {/* Edge */}
      <div className="num col-span-2 min-w-0 truncate text-meta text-ink-2 lg:col-span-1" title={`${d.edge_id}${d.adapter ? ` · 设备类型 ${d.adapter}` : ''}${d.port ? ` · ${d.port}` : ''}`}>
      ...
      <div className="col-span-2 flex min-w-0 flex-wrap items-center gap-x-3 gap-y-1 lg:hidden xl:col-span-1 xl:flex">
        {!descriptor ? (
          <span className="text-meta text-ink-3" title="设备尚未同步信息，读数未知">读数等待同步</span>
        ) : metrics.length === 0 ? (
          <span className="text-meta text-ink-3" title="设备信息里没有可显示的主读数">暂无可读数值</span>
        ) : (
          metrics.map((m, i) => (
      ...
      <div className="num hidden min-w-0 truncate text-right font-mono text-micro text-ink-3 lg:block" title={seen ? fmtDateTime(seen) : '从未更新'}>
