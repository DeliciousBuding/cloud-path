  ...
  return (
    ...
    >
      {isLoading ? (
        ...
        <p className="py-4 text-center text-body text-ink-3">这台设备还没有操作记录</p>
      ) : (
        <>
          ...
          {shown.map((c) => {
            ...
            return (
              <li key={c.id} className="grid min-w-0 grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-x-3 gap-y-1 py-3">
                ...
                {progress && (
                  <p className="col-span-3 min-w-0 text-meta text-ink-3">{progressCopy(c.status)}，请勿重复操作。</p>
                )}
                  ...
                  <div className="col-span-3 flex min-w-0 flex-wrap items-center gap-x-2 gap-y-2 rounded-tile bg-bad/8 px-2.5 py-2 text-meta">
                    ...
                    {!online ? (
                      ...
                      <CommandButton deviceId={deviceId} targetLabel={targetLabel} action={action} args={c.args ?? ''}
                        buttonLabel="重试" buttonAriaLabel={`重试${meta.label}`} className="btn-sm min-h-touch w-full sm:min-h-0 sm:w-auto" />
                    ) : (
                      <Link to={controlPath(deviceId)} className="link inline-flex min-h-touch shrink-0 items-center sm:min-h-0">去设备操作中重试</Link>
                    )}
                  </div>
                )}
                ...
                <details className="col-span-3 min-w-0 text-meta">
                  <summary className="flex min-h-touch cursor-pointer select-none items-center text-ink-3 sm:min-h-0">技术详情</summary>
