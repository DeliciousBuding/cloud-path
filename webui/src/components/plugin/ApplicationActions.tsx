...
function ActionResult({ json }: { json: string }) {
  ...
  return <div className="mt-3 min-w-0 space-y-3">
    ...
    {json && <details className="min-w-0">
      <summary className="flex min-h-touch cursor-pointer items-center text-meta text-ink-2">查看结果原文</summary>
...
function ActionForm({ instanceID, job, scope, schema, enabled, hasInput }: {
  ...
  return <div className="min-w-0">
    ...
    {mutation.isError && <div className="mt-3 min-w-0 space-y-2 rounded-tile border border-hairline p-3">
      ...
      <p className="text-meta leading-relaxed text-ink-2">不会自动重试。请先查看应用记录；再次执行会沿用上次内容，修改参数后再执行会作为一次新的操作。</p>
      {mutation.error instanceof Error && mutation.error.message && <details className="min-w-0">
        <summary className="cursor-pointer text-meta text-ink-2">错误详情</summary>
    ...
    {mutation.isSuccess && <div className="mt-3 min-w-0 rounded-tile border border-hairline p-4">
      <p role="status" className="text-body font-medium">操作已受理</p>
      <p className="mt-1 text-meta leading-relaxed text-ink-3">以下为应用返回的结果，不一定代表设备已经执行。设备执行结果请在应用记录中核对。</p>
      <h4 className="mt-4 text-body font-medium">执行结果</h4>
      <ActionResult json={mutation.data.result_json} />
    </div>}
    {request && <details className="mt-3 min-w-0">
      <summary className="flex min-h-touch cursor-pointer items-center text-meta text-ink-2">本次请求详情</summary>
      <p className="mt-2 break-all text-meta text-ink-2">请求标识：<code>{request.idempotency_key}</code></p>
...
export function ApplicationActions({ instanceID, jobs, running, conflict = false, desiredEnabled = true, lifecycleKey }: {
  ...
  return <div className="min-w-0">
    <p className="mb-3 text-meta leading-relaxed text-ink-3">这里显示可以手动执行的操作。后台定时任务会自动运行。</p>
    {!scope && <p className="mb-3 text-body text-ink-2">当前账号只能查看，不能执行操作。</p>}
    ...
    {actions.length ? <div className="divide-y divide-hairline">{actions.map((job) => <Action
      ...
      : jobs && <p className="py-3 text-body text-ink-3">暂无应用操作</p>}
  </div>
}
