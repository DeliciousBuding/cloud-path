...
export function SyncBanner({ v }: { v: PluginInstanceView }) {
  ...
  return (
    <div className={`flex min-w-0 items-start gap-2.5 rounded-tile px-3.5 py-3 ${boxCls}`} role="status">
      <span className={`mt-0.5 shrink-0 ${fgCls}`}>
        <Icon size={15} />
      </span>
      <span className="min-w-0 flex-1">
        <span className="block text-compact font-semibold break-words">{s.label}</span>
        <span className="mt-0.5 block text-meta leading-relaxed break-words text-ink-2">{s.hint}</span>
      </span>
    </div>
  )
}
...
function Row({ k, v, tone, wrap }: { k: string; v: ReactNode; tone?: 'ok' | 'warn' | 'bad' | 'idle' | 'accent'; wrap?: boolean }) {
  ...
  return (
    <div className="flex min-w-0 items-baseline justify-between gap-2 border-b border-hairline py-1.5 last:border-b-0">
      <dt className="shrink-0 text-meta text-ink-3">{k}</dt>
      <dd className={`num min-w-0 text-meta font-medium ${wrap ? 'break-words' : 'truncate'} ${cls ?? ''}`}
...
function ColumnHead({ children, note }: { children: ReactNode; note: string }) {
  return (
    <div className="mb-1.5 flex min-w-0 items-baseline justify-between gap-2">
      <h3 className="min-w-0 truncate text-meta font-semibold tracking-[-0.01em]">{children}</h3>
...
function UnreportedBlock({ v }: { v: PluginInstanceView }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center rounded-tile bg-surface px-3 py-6 text-center">
      <span className="flex h-9 w-9 items-center justify-center rounded-pill bg-ink-3/12 text-ink-2">
        {v.edge_online ? <RadioTower size={16} /> : <CloudOff size={16} />}
      </span>
      <p className="mt-2.5 text-compact font-semibold">{v.edge_id === 'server' ? t('desired.noState') : t('desired.noObserved')}</p>
      <p className="mt-1 text-meta leading-relaxed break-words text-ink-2">
  ...
  return <div className="space-y-4">
    <div className="flex min-w-0 items-start gap-3">
      ...
      <div className="min-w-0">
        ...
        <p className="mt-2 break-words text-body font-medium">{status.summary}</p>
        {next && <p className="mt-1 break-words text-meta leading-relaxed text-ink-2">{t('desired.next', { text: next })}</p>}
      </div>
    </div>
    <dl className="grid gap-3 border-t border-hairline pt-4 sm:grid-cols-3">
      <div className="min-w-0">
        <dt className="text-meta text-ink-3">{t('desired.summaryLocation')}</dt>
        <dd className="mt-1 break-words text-compact font-medium">{instanceLocationLabel(v)}</dd>
      </div>
      <div className="min-w-0">
        <dt className="text-meta text-ink-3">{t('desired.summaryState')}</dt>
        <dd className="mt-1 break-words text-compact font-medium">{v.has_observed ? state.label : t('common.statusUnknown')}</dd>
      </div>
      <div className="min-w-0">
        <dt className="text-meta text-ink-3">{t('desired.summaryHealth')}</dt>
        <dd className="mt-1 break-words text-compact font-medium">{v.has_observed ? health.label : t('desired.summaryNotReceived')}</dd>
      </div>
    </dl>
  </div>
}
...
export function DesiredObserved({ v }: { v: PluginInstanceView }) {
  ...
  return (
    ...
    <div className="grid grid-cols-1 gap-2.5 sm:grid-cols-2 sm:gap-4">
      <div className="min-w-0 rounded-tile bg-surface-2 p-3 sm:p-3.5">
      ...
      <div className="min-w-0 rounded-tile bg-surface-2 p-3 sm:p-3.5">
        ...
        {v.has_observed ? (
          <>
            <dl className="m-0">
              <Row k={t('sections.state')} v={st.label} tone={st.tone === 'idle' ? undefined : st.tone} />
            ...
            {v.stale && (
              <p className="mt-2 flex items-start gap-1.5 rounded-tile bg-warn/12 px-2.5 py-2 text-meta leading-relaxed text-warn">
            ...
            {v.observed?.detail && hostDetailLabel(v.observed.detail) !== v.observed.detail && (
              <p className="mt-2 truncate text-meta text-ink-3" title={v.observed.detail}>
    ...
    <details className="min-w-0 text-meta text-ink-2">
      <summary className="flex min-h-touch cursor-pointer items-center">{t('desired.technicalDetails')}</summary>
