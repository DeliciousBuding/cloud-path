...
function UnreportedBlock({ v }: { v: PluginInstanceView }) {
  return (
    <div className="flex flex-col items-center justify-center rounded-tile bg-surface px-3 py-6 text-center">
      ...
      <p className="mt-2.5 text-compact font-semibold">{v.edge_id === 'server' ? '还没有运行状态' : '还没有收到运行状态'}</p>
  ...
  return <div className="space-y-4">
    <div className="flex min-w-0 items-start gap-3">
      ...
      <div className="min-w-0">
        ...
        {next && <p className="mt-1 break-words text-meta leading-relaxed text-ink-2">下一步：{next}</p>}
      </div>
    </div>
    <dl className="grid gap-3 border-t border-hairline pt-4 sm:grid-cols-3">
      <div className="min-w-0">
        <dt className="text-meta text-ink-3">运行位置</dt>
        <dd className="mt-1 break-words text-compact font-medium">{instanceLocationLabel(v)}</dd>
      </div>
      <div className="min-w-0">
        <dt className="text-meta text-ink-3">当前状态</dt>
        <dd className="mt-1 break-words text-compact font-medium">{v.has_observed ? state.label : '状态待确认'}</dd>
      </div>
      <div className="min-w-0">
        <dt className="text-meta text-ink-3">健康情况</dt>
        <dd className="mt-1 break-words text-compact font-medium">{v.has_observed ? health.label : '尚未收到'}</dd>
      </div>
    </dl>
  </div>
}
