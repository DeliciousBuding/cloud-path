    ...
    <>
      ...
      {manual.length > 0 && (
        <details className={simple.length > 0 || parameterized.length > 0 ? 'mt-4 border-t border-hairline pt-4' : ''}>
          <summary className="flex cursor-pointer select-none items-center gap-1.5 text-[12px] font-medium text-ink-3 transition-colors hover:text-ink-2">
...
export function ActionPanel({ deviceId, set, className }: {
  deviceId: string
  set: CommandSet
  className?: string
}) {
