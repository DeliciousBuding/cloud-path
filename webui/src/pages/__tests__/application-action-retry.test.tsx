...
describe('应用操作只在明确用户请求时重试', () => {
  ...
  it('编辑参数即新语义，即使改回原值也不复用失败请求 key', async () => {
    ...
    const input = await screen.findByRole('combobox', { name: /次数/ })
  ...
  it('仅切换 JSON/字段视图不是新请求；重试保留原始空白与 key', async () => {
    ...
    const user = userEvent.setup()
    renderWithProviders(<ApplicationPlane instanceID="app-a" />)
    await screen.findByRole('combobox', { name: /次数/ })
    await user.click(screen.getByRole('button', { name: '更新计数 高级参数' }))
    const args = JSON.stringify({ count: 2 }, null, 2)
    fireEvent.change(screen.getByRole('textbox', { name: '更新计数 参数内容' }), { target: { value: args } })
    await user.click(await button('执行「更新计数」'))
    await screen.findByText(/操作发生冲突/)
    await user.click(screen.getByRole('button', { name: '更新计数 改用表单填写' }))
  ...
  it.each([
    ...
    expect(actionRequests(http)).toHaveLength(1)
    fireEvent.click(screen.getByText('错误信息'))
