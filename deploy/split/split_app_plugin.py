#!/usr/bin/env python3
"""Reproduce a historical bootstrap snapshot of the reference Application plugin.

Bootstrap input: ``examples/scheduled-compartment`` in this monorepo, NOT the
...
* Default output uses the **published module path** with a historical version
  default (not the maintained application SDK baseline)::
...
* ``--core-path <dir>`` additionally writes a ``replace`` directive pointing at
  a local core checkout for scratch verification without changing any maintained
  application dependency. A tree
...
def render_readme(upstream: str) -> str:
    ...
    header = (
        f"# {PLUGIN_REPO_SLUG}\n\n"
        "> Historical bootstrap snapshot of the reference **Application** plugin\n"
        "> `scheduled-compartment`, generated by `deploy/split/split_app_plugin.py`.\n"
        "> Not the maintained application source; never regenerate over its independent repository.\n\n"
...
def main(argv: list[str]) -> int:
    ...
    parser.add_argument("--force", action="store_true", help="replace an existing scratch output directory (not a maintained plugin checkout)")
    ...
    print("")
    print("Historical first-bootstrap steps only; NOT an update path for the maintained repository")
    ...
    print(f"  gh repo create DeliciousBuding/{PLUGIN_REPO_SLUG} --public --source . --remote origin  # first creation only; never reuse an existing application repo")
